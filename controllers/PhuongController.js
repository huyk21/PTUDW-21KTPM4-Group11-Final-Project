...
const showAd = asyncHandler(async (req, res) => {
  try {
    ...
    res.locals.adboard = adboard.filter(
...
const editAd = asyncHandler(async (req, res) => {
  ...
  const details = adboard.filter(
    (ad) => ad.location._id.toString() === adID.toString()
  );
  res.render("adManager_modal", {
...
const showLicense = asyncHandler(async (req, res) => {
  try {
    ...
    const workWard = req.session.workWard;
    const ward = await Ward.findById(workWard);
    res.locals.license = license.filter(
      (li) => li.location.ward.toString() === workWard
    );
...
const showConfirm = asyncHandler(async (req, res) => {
  ...
  const wardLicenses = licenses.filter(
    (li) => li.location.ward.toString() === workWard
  );
  const results = licenses.filter((li) => li._id.toString() === id);
  res.render("adLicense_modal", {
    layout: "layoutAdLicense",
    license: wardLicenses,
    selected: results[0],
    ward: ward,
  });
});
...
const showReport = asyncHandler(async (req, res) => {
  try {
    ...
    res.locals.report = report.filter(
      (rp) => rp.ward._id.toString() === workWard
    );
...
const showReportDetails = asyncHandler(async (req, res) => {
  ...
  const details = report.filter(
    (rp) => rp._id.toString() === reportID.toString()
  );
  res.render("reportManager_modal", {
    layout: "layoutReportManager",
    report: result,
    details: details[0],
    ward: ward,
